...
//ANthony added
assign outbuffer = {5'b00000,g_x_min};
//end
////////////////////////////////////////////////////////////////////////
...
reg [7:0] pix_r1, pix_r2, pix_r3, pix_r4;
reg [7:0] pix_g1, pix_g2, pix_g3, pix_g4;
reg [7:0] pix_b1, pix_b2, pix_b3, pix_b4;
...
initial begin
    ...
    // prev_p2 <= 0;
    // prev_p3 <= 0;
    //rohan additions (remove above if works)
    pix_r1 <= 0;
    pix_r2 <= 0;
    pix_r3 <= 0;
    pix_r4 <= 0;
...
always @(*) begin
    sum_r = (4*pix_r1 + 3*pix_r2 + pix_r3);
    average_r = sum_r / 8 ;
    sum_g = (4*pix_g1 + 3*pix_g2 + pix_g3);
    average_g = sum_g / 8;
    sum_b = (4*pix_b1 + 3*pix_b2 + pix_b3);
    average_b = sum_b / 8;
	// sum_r = (7*pix_r1 + 6*pix_r2 + 3*pix_r3);
    // average_r = sum_r >> 3 ;
    // sum_g = (9*pix_g1 + 5*pix_g2 + 2*pix_g3);
    // average_g = sum_g >> 3;
    // sum_b = (9*pix_b1 + 5*pix_b2 + 2*pix_b3);
    // average_b = sum_b >> 3;
...
assign detected_area  =  (red_detect) ? {8'hff, 8'h0, 8'h0} : 
						  (green_detect) ? {8'h0, 8'hff, 8'h0} :
						 (pink_detect) ? {8'h9B, 8'h06, 8'hB8} :
						 {8'h0, 8'h0, 8'h0};
...
wire red_bb_active, green_bb_active, pink_bb_active, build_bb_active;
...
assign new_image = red_bb_active ? {8'hdf,8'h4d,8'h14} : 
					 ...
					 build_bb_active ? {8'h1E,8'hF1,8'h13} :
                     detected_area;
...
always@(posedge clk) begin
    ...
    else if (green_detect & in_valid) begin	//Update bounds when the pixel is GREEN
		if (x < g_x_min) g_x_min <= x;
		if (x > g_x_max) g_x_max <= x;
		if (y < g_y_min) g_y_min <= y;
		g_y_max <= y;
	end
	else if (pink_detect & in_valid) begin	//Update bounds when the pixel is pink
		if (x < p_x_min) p_x_min <= x;
		if (x > p_x_max) p_x_max <= x;
		if (y < p_y_min) p_y_min <= y;
		p_y_max <= y;
	end
	// else if (////???????////// & in_valid) begin	//Update bounds when the pixel is part of a building...
	// 	if (x < build_x_min) build_x_min <= x;
	// 	if (x > build_x_max) build_x_max <= x;
	// 	if (y < build_y_min) build_y_min <= y;
	// 	build_y_max <= y;
	// end
...
always@(*) begin	//Write words to FIFO as state machine advances
	case(msg_state)
		...
		3'b100: begin
			msg_buf_in = {8'b00000100,1'b0,build_x_min,1'b0,build_x_max};	//BUILDING Colour ID, 0-bit + g_x_max, 0-bit + g_x_min
			msg_buf_wr = 1'b1;
		end
