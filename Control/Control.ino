...
float dcoeff = 0.11; //was 0, then 0.2
float multiplier = 1;
//
//float pcoeff = 3.4;
//float icoeff = 0.02;
...
void Opticaltaskcode(void *param){
 ...
 if (mousecam_init() == -1)
  ...
  for(;;){
    ...
    total_x = total_x1 * 0.1008;   //callibrated;
    total_y = total_y1 * 0.0235;  //callibrated;
  }
}
void setup()
{
...
// void sendtoServer(int op,uint16_t first , uint16_t second){ //changed first and second to ints
...
//  //sendtoSever(3,0,0)
// }
void sendtoServer(int op,int first , int second){ //changed first and second to ints
      uint16_t first_msg, second_msg;
      if(first < 0){
        first_msg = -first;
        first_msg+=32768;
      }
      ...
      memset(values, 0, 4); //just resetting the buffer
      values[0] = op;
      values[1] = (first_msg & 65280)>>8;
      values[2] = first_msg & 255;
      values[3] = (second_msg & 65280)>>8;
      values[4] = second_msg & 255;
      client.write(values, 5);
      delay(100);
 //sendtoSever(3,0,0)
}
void loop()
{ 
  if(building_done == true & millis()-timee > 10000){
    building_done = false;
  }
  //  if((counter % 5) == 0){
...
// Listening to vision:
    ...
    digitalWrite(15, LOW);
    if(fourteen_reply == false){
        spi_val = MySPI.transfer16(0);
    }
    else{
        spi_val = MySPI.transfer16(14);
    }
    spi_returnval = 0;
    ...
    if (spi_val == 9 || spi_val == 10) { 
        angle_state = true;
        if(object_measuring == false){
          Serial.println("now changing position");
          robot.brake(1);
          robot.brake(2);
          angle_t = total_x;
          ...
          Serial.println("started changing position");
          if(spi_val == 9){
            angle_t = total_x - 2;
          }
          else{
    ...
    //no data
        ...
        if(object_measuring){
            ...
            sendtoServer(1,0,0);
            if(fourteen_reply == true){
                fourteen_reply = false;
            }
        }
    }
    else if  (spi_val==13){
        ...
        if(object_measuring == false & building_done == false ){
            fourteen_reply = true;
            object_measuring = true;
            robot.brake(1);
            robot.brake(2);
...
// //////Receiving new command list from the server...//////////////
  ...
  if (!object_measuring && client.available())
  {
    ...
    if(opcode==0){
        ...
        if(magnitude > 180 & magnitude < 360){
