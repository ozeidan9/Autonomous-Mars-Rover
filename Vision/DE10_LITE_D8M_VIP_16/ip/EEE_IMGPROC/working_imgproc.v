...
assign detected_area  =  (red_detect & prev_r1 & prev_r2 & prev_r3) ? {8'hff, 8'h0, 8'h0} : 
						...
						//  (black_detect & (x>7) & (x<640-7) & (y>7) & (y < 480-7) & prev_b1 & prev_b2 & prev_b3) ? {8'hd2, 8'h85, 8'h0} : 
...
assign new_image =	red_bb_active ? {8'hdf,8'h16,8'h0} : 
                    // black_bb_active ? {8'hdb,8'hdf,8'h00} : 
...
reg[3:0] max_number_of_stripes;
reg[3:0] current_number_of_stripes;
reg[3:0] prev_frame_number_of_stripes;
reg left_building, right_building;
...
always@(posedge clk) begin
    ...
    else if (white_detect & prev_w1 & prev_w2 & prev_w3 & in_valid) begin    //Update bounds when the pixel is WHITE
        if (x < w_x_min) w_x_min <= x;
        if (x > w_x_max) w_x_max <= x;
        if (y < w_y_min) w_y_min <= y;
        w_y_max <= y;
    end
    if (building_detect & in_valid) begin    //Update bounds for BUILDING
...
always@(*) begin    //Write words to FIFO as state machine advances
    case(msg_state)
		...
		3'b100: begin
		 	msg_buf_in = {8'b00000010,1'b0,r_y_min,1'b0,r_y_max};	//Red Colour ID, 0-bit + r_x_max, 0-bit + r_x_min
			msg_buf_wr = 1'b1;
		end
		3'b101: begin
		 	msg_buf_in = {8'b00000010,1'b0,r_y_min,1'b0,r_y_max};	//Red Colour ID, 0-bit + r_x_max, 0-bit + r_x_min
