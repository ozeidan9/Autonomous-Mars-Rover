...
//This is the buffer for sending to the server (it has has 5 elements)
uint8_t values[4];
...
//    }
    ...
    sendtoServer(0, 65484, 8396); //position sending with opcode 0;

    // Serial.print("Bool log");
    // Serial.println(!distance_state && !angle_state);
    if(!distance_state && !angle_state){ //should be 
      delay(100);
      Serial.println("NOT DIST NOR ANGLE");
      sendtoServer(1, 0, 0); //send a UPM message to server...
      Serial.println("UPM is sent");
      delay(100);
    }
    else{
      sendtoServer(3, 0, 0);
...
//      }
    ...
    if(opcode==0){
        ...
        angle_state = true;
        Serial.println("ANGLE SET TO TRUE");
    ...
    if(opcode==1){
        ...
        Serial.println("DISTANCE SET TO TRUE");
      ...
      if(total_y > distance_t + 1 | total_y < distance_t - 1 && distance_state  &&!angle_state){
        distance_reached = 0;
...
//        ierror = error*(pathnow - pathstart) + ierror;
      ...
      //-360 to 0 : 361 is -1, 362 is -2
       ...
       //REMOVE BELOW LATER PLSSSSS
        Serial.println("done moving");
        distance_state = false;   //RETURN LATER PLSSSSSS
        client.write("UPM");
     ...
     if(total_x > angle_t + 1 | total_x < angle_t - 1 && angle_state &&!distance_state){
       angle_reached = 0;
       Serial.println("not within range");
