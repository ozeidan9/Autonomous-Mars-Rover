...
// Battery 
long int lasttime = millis(); //this needs to be taken as an input for the function below, preferably in the actual arduino setup block
float batdrain = 0; //this also works well in the actual arduino setup block
bool radar_enable = false;
long int nowtime;
int batpercent;

//battery
int batperc(float batdrain) { //the actual function, like "batpercent = batperc(batdrain);  SEND batpercent;". Stick this before loop, I just stuck it here so you can see integration
    int batcap = 90000;
    float batlev = ((batcap - batdrain)/batcap)*100; //calculate the percentage
    int batpercout = batlev;
    return batpercout;
}
...
//Bat level
  nowtime = millis(); //setup every time you call the function
  int diftime = nowtime - lasttime;
...
// Listening to vision:
    ...
    if((spi_val == spi_val_prev1 & spi_val == spi_val_prev2)| (spi_val > 30 & spi_val_prev1 < spi_val + 5 & spi_val_prev1 > spi_val - 5)){
      vision_msg = spi_val;
...
////////Receiving new command list from the server...//////////////
  ...
  if (!object_measuring && client.available())
  {
    ...
    //Dista
      ...
      if(radar_enable){ //radar on
        batdrain = batdrain + diftime*3.4732/1000;
      }
      else if (!radar_enable){ //radar off
        batdrain = batdrain + diftime*3.2632/1000;
      }
      

      if(millis()%1000){
        batpercent = batperc(batdrain);
        Serial.print("Battery level is: ");
        Serial.println(batpercent);
        lasttime = millis(); 
        sendtoServer(4, 0, batpercent);
      }
